...
import { $ } from 'execa';
import { getJrePath, getJreBin, fetch } from "../lib/utils.js";
...
if (args.includes('--help') || args.includes('-h')) {
    console.log(`
Usage: fhir-install-java [options]
...
Description:
  Downloads JRE 21 from adoptopenjdk.net and installs it in the
  project's jre/ folder. The FHIR validator requires Java 17+.

Exit codes:
  0 - JRE installed or already available
...
const move = (file) => {
  const newFile = path.join(process.cwd(), file.split(path.sep).slice(-1)[0]);
